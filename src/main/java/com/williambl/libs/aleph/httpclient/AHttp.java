...
public class AHttp {
     ...
     */
    public static <T> Either<HttpResponse<T>, HttpStatusFailure<T>> expectStatus(HttpResponse<T> response, Predicate<Integer> predicate, String expectedCodeName) {
        return predicate.test(response.statusCode()) ? Either.left(response) : Either.right(new HttpStatusFailure<T>("Expected status %s from HTTP response, got status %s.".formatted(expectedCodeName, response.statusCode()), expectedCodeName, response.statusCode(), response));
    }

    /**
     * Convenience overload for {@link #expectStatus(HttpResponse, Predicate, String)} for a single status.
     */
    public static <T> Either<HttpResponse<T>, HttpStatusFailure<T>> expectStatus(HttpResponse<T> response, int status) {
